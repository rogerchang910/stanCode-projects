...
-----------------------------------------------
This program will generate a new image (without pedestrian)
by selecting pixels in the same position of several images
input and choose the best pixel.
"""
...
def get_best_pixel(pixels):
    ...
    pixel = pixels[0]
    d = get_pixel_dist(pixel, avg[0], avg[1], avg[2])

    for i in range(len(pixels)):
        best_pixel = pixels[i]
        dist = get_pixel_dist(best_pixel, avg[0], avg[1], avg[2])
        if dist <= d:
            pixel = best_pixel
